...
The React team has developed a wonderful script for starting a new React app. The command will be<br>`npx create-react-app <app name>` - think of it as the express-generator for React apps.
...
However, before we use `npx create-react-app` we will use a most excellent code playground, [CodeSandbox](https://codesandbox.io/) to take a first look at how React works...
...
We will be using `npx create-react-app` to scaffold all React apps in SEI from this point forward.
