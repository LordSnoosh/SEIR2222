...
constructor() {
  ...
  this.state = {
    selColorIdx: 0,
    // Update to initialize with only one guess object
...
#### 💪 Exercise - Passing Props (3 mins)

1. Using the Component explorer of React Developer Tools and beginning with the parent component of `<GuessPeg>`, keep going up the component hierarchy until you find the component that has a `currentGuess` prop.
 
2. Then pass down the `currentGuess` prop through the component hierarchy - ultimately to each `<GuessPeg>` component.
