...
Let's check our code by signing up a new user and using a [Node REPL session](https://gist.github.com/jim-clark/57b646abbb6c0ce09f9fa948eab6febc) to check that the password has been hashed:

The users' passwords should now be hashed!
...
In react-mastermind, none of the routes defined in the users router depend upon there being a logged in user, so let's mount our new middleware right after those routes in **server.js** like this:
...
The above code skips checking for a token and creating `req.user` when handling any of the **user** related routes (signing up or logging in/out).
