...
The starter code is the React Mastermind app that includes the timer implemented from the previous lesson and lab.

- `cd ~/code/react-mastermind`
- `git fetch --all`, then `git reset --hard origin/master`
...
Just to be clear, the Express backend is configured and ready for additional backend functionality to be coded.

When the time comes, be sure to add folders such as `config`, `routes`, `models` & `controllers` to keep your backend code organized.

Additionally, in a SPA, the routes will be API-type routes, i.e., they should be namespaced using `/api` and respond with JSON, not EJS views, i.e., `res.json(...)` instead of `res.render(...)`.
