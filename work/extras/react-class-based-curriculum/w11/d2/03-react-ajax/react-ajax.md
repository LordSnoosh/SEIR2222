...
By default, `fetch` participates in CORS - thus, providing an options object with a `mode: "cors"` property like the following is optional:

```js
// `{mode: 'cors'}` is the default
...
The API requires an API Key to use, yours truly will provide you with a key to prevent you from having to register for one.
...
```
https://api.openweathermap.org/data/2.5/weather?lat=34.0475869&lon=-117.8985651&units=imperial&appid=<key>
```
